...
The flip side of "anyone can write" is that anyone can write anything: NSFW material, obscenity, spam, off-topic garbage. Centralized platforms (Facebook, YouTube, Meta at large) solve this by employing moderators in large numbers, increasingly assisted by AI — paid out of a corporate budget. A decentralized system has **no corporation and no budget**. Consuming unmoderated feeds means consuming out-of-control filth, which makes the whole publishing layer unusable for ordinary applications.
...
### 3.3 Round one — a random subset votes, first come first counted

When a submission arrives, a **random subset of moderators** becomes eligible to vote — a working value of 1–10% of the moderator set, tightening as the set grows. The subset is drawn stake-weighted (splitting one stake into many identities buys no extra presence) over the moderator set *as it existed before the submission block*, with a ~1-week activation delay on new stake, seeded by on-chain randomness snapshotted a few blocks *after* submission. Nobody — submitter or moderator — can position themselves for a specific case, and staking three moderators to approve your own content stops working the moment the moderator set has any size.

Eligibility over a subset (rather than a fixed drawn panel) is a deliberate liveness choice: some eligible moderators will be offline, but with 1000 moderators a 20–30 strong subset almost always contains several who are online *right now*, so cases move fast.

Voting is **commit-reveal, adapted for speed**: eligible moderators submit `H(vote, salt)`, and the commit phase ends as soon as the **first 5 commits** land (or after 24h, whichever comes first). A short reveal window follows. Commits that fail to reveal take a brief freeze and don't count; if fewer than 3 votes reveal, the subset widens and voting reopens. Commit-reveal keeps the five judgments independent — public votes would invite copying the first one.

### 3.4 Probabilistic outcomes and bonded appeals

The revealed votes are tallied by **the stake behind each side**, and the round's outcome is **drawn with probability proportional to stake**: if approve-stake is twice reject-stake, approve wins with 2/3 probability. A unanimous round needs no luck — one side holds all the stake. (Swarm's storage incentives use the same family of mechanism: stake-proportional probabilistic winner selection.)

The result is preliminary. An **appeal window** follows (working value: 3 days; the first window 4 days, so long holiday weekends cannot decide outcomes). During the window, *anyone* — a moderator, the submitter whose content was rejected, an uninvolved third party — may appeal by posting a **bond**, escalating with depth (working value: each bond ≥ 2× the previous round's total reward). An appeal draws a **fresh, larger random subset** (working sizes: 5 → 11 → 23 counted votes), which votes the same way, and the outcome is again drawn probabilistically.
...
**First-come voting dynamics.** "First 5 commits count" rewards fast moderators — mostly AI — and speed-of-response is not the same as quality-of-judgment. Accepted for MVP liveness; worth revisiting if racing distorts outcomes.
