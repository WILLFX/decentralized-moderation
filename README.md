...
- the **supersafe subset** — the startpage mode: `uncontested && fullQuorum && now − approvalTime ≥ 96h`. Judged safe with not a single dissenting vote, decided by a full panel of independent revealers rather than an under-quorum fallback, and sat through the first appeal window with nobody in public objecting — as close to certainty as a decentralized system gets, computed entirely client-side from the same fields.
