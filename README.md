...
- the **supersafe subset** — the startpage mode: `uncontested && fullQuorum && now − approvalTime ≥ 96h`. No `Reject` vote was ever revealed against it in any round, it was decided by a full cohort rather than a thin one, and it has stood approved for 96 hours — the length of the first challenge window, so an approval that was going to be overturned has, in the ordinary case, already been overturned and removed. As close to certainty as a decentralized system gets, computed entirely client-side from the same fields.
...
| 3 | **Submit interface** | Web GUI for content creators: compose submission (content hash, metadata JSON validated against the schema, topics), pay fee, track status, challenge rejections | Rust → WebAssembly |
| 4 | **Search dapp** | Safe-search front end: query the approved index by topic via contract view functions; supersafe startpage mode (§3.8) and full superset view; ranking and presentation live client-side and are replaceable | Rust → WebAssembly |
