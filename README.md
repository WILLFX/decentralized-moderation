...
**Easy way (MVP):** when a case **settles** as an approval, its entry is written to an **in-contract map: topic → vector of approved entries** (writing at settlement rather than at the first round's tally is what lets an approval *won on appeal* be indexed at all, and keeps provisional entries out of the index entirely). Each entry carries the content hash, the metadata hash, the **time of approval**, and two booleans: **`uncontested`** and **`fullQuorum`**.

`uncontested` is `true` **iff no `Reject` vote was ever revealed, in any round of the case**. An **appeal alone does not clear it**: a frivolous appeal that draws a fresh panel which again reveals no reject vote leaves the entry uncontested. The flag marks entries *no dissenting voter ever opposed*, and a unanimous round involves no probabilistic draw to get lucky in — so the "snuck back in on a lucky draw" concern it guards against cannot arise. Clearing it on the mere *act* of appealing would instead hand any vandal a way to exclude any entry from the safe view permanently, for the price of one abandoned appeal. `specs/state-machine.md` §8.1 is the normative statement.

`fullQuorum` is `true` iff no round fell back to deciding below `MIN_REVEALS` after the widen cap, and the deciding round drew at least `MIN_REVEALS` **independent revealers** — addresses, not seats, so one multi-seat voter cannot satisfy quorum alone. If an appeal ends in rejection, the entry is removed at settlement. A submission with multiple topics costs proportionally more, since more contract storage is written. The search dapp then serves queries entirely from **contract view functions against the latest state** — no scraping of historical logs, no off-chain indexer infrastructure.
...
- the **supersafe subset** — the startpage mode: `uncontested && fullQuorum && now − approvalTime ≥ 96h`. No `Reject` vote was ever revealed against it in any round of the case, it was decided by a full panel of independent revealers rather than an under-quorum fallback, and it has stood approved for 96 hours — the length of the first appeal window, so an approval that was going to be overturned has, in the ordinary case, already been overturned and removed. Note the age term measures survival, not silence: an entry *may* have been appealed within that window (an appeal alone does not clear `uncontested`, as above) and may still be under appeal at the moment it qualifies. As close to certainty as a decentralized system gets, computed entirely client-side from the same fields.
...
| 4 | **Search dapp** | Safe-search front end: query the approved index by topic via contract view functions; supersafe startpage mode (uncontested + full-quorum + seasoned entries, §3.8) and full superset view; ranking and presentation live client-side and are replaceable | Rust → WebAssembly |
...
**M2 — Contract.** *Implemented (`contracts/`, Foundry).* Solidity implementation of the full state machine — staking and the free/committed/frozen partition, a stake-weighted sortition tree (a clean 0.8.x port of Kleros's MIT sum-tree), the case lifecycle with two-seed randomness, bonded appeals, the solvent settlement order, the index, and timelocked governance. 266 tests including a handler-driven invariant campaign (funds conservation, no internal transfer), a 52-vector differential regression test against an integer reference (a port of the contract, so it catches drift rather than certifying the formula), the single-stake-benefit statistical property, and gas-bound tests (worst-case settlement ~2.5M gas, under an 8M single-transaction ceiling). Spec departures are catalogued in `contracts/DEVIATIONS.md`. The contract defines the protocol; everything else is a client of it.
