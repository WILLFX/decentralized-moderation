...
MUTATIONS = [
 ...
 ("M11","§8.1","listing membership no longer tracks APPROVED",
  ...
  "        bool shouldBeListed = (status != uint8(Status.NONE) && actionType == ACTION_LIST);"),
 ...
 # --- M2.10: actionType on the entry, and the fifth write's reach -----------
 ("M27","§8.2/M2.10","the listing predicate drops the actionType conjunct - a successful REMOVAL lists itself",
  "        bool shouldBeListed = (status == uint8(Status.APPROVED) && actionType == ACTION_LIST);",
  "        bool shouldBeListed = (status == uint8(Status.APPROVED));"),
 ("M28","§8.3/M2.10","removeListing leaves `strict` set - a REMOVED entry reads SUPER_SAFE",
  "        e.strict = false;\n\n        _syncListing",
  "\n        _syncListing"),
 ("M29","§8.1/M2.10","the fifth write may retarget a REMOVE entry",
  "        if (e.actionType != ACTION_LIST) revert NotAListEntry();\n",
  ""),
 ("M30","§8.2/M2.10","the entry does not record what its claim asked",
  "        e.actionType = actionType;\n",
  ""),
 ("M32","§8.2/M2.10","an out-of-range actionType is accepted",
  "        if (actionType > ACTION_REMOVE) revert BadActionType();\n",
  ""),
 ("M31","§8.2/M2.10","the two action types collapse to one value",
  "    uint8 public constant ACTION_REMOVE = 1;",
  "    uint8 public constant ACTION_REMOVE = 0;"),
