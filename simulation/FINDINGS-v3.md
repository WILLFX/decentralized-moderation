...
`specs/design-v3.md` and `specs/protocol.md`.

**Nothing from `simulation/` carries over.** Its §A and §D are produced by risk
...
every parameter in `specs/protocol.md` §10.
...
`specs/protocol.md` §4.5 claims the single-randomness rule largely removes the
...
`specs/protocol.md` §3.3 argues widening cannot shift composition because it is
...
> 250 with no safety gain at either `prior`, and `specs/protocol.md` §4.8b deleted
...
implements what the spec specifies (`specs/protocol.md` §4.5), and the spec keeps
...
content. `specs/protocol.md` §10 carries the question of dropping it.

---

## H. Permanence, and how far the recourse actually reaches

`specs/protocol.md` §8.4 makes `REJECTED` permanent and §8.5 makes a re-review the
