# Moderation Protocol — Formal State-Machine Specification

**Status:** Normative. This is the only design document on `main`; where anything
else in the repository disagrees with it, this file wins.
...
carries one design, per review feedback. They resolve on the
`archive/v1-v2-and-design-history` branch.
...
| ~~`MIN_COMMITS`~~ | — | **Removed, §4.8b.** It was an absolute count gating a cohort proportional to a registry size §3.6 forbids observing, so it was inert wherever the registry exceeded its calibration and destructive below. Measured (`simulation/FINDINGS-adaptive.md` §2): identical outcomes for every value 1–16 at registry ≥ 1,000, and 92% `NO_TURNOUT` at 250 with no safety gain at either `prior`. The gate a thin tally needs is on **confidence**, and §4.5's `â` is already it. |
...
Found by implementation (`contracts/src/StakeRegistry.sol`), not by reading —
...
**What it costs.** `simulation/FINDINGS-v3.md` §G measures it. False rejection of
...
**Source: `simulation/FINDINGS-v3.md` §B, at the commit that last touched
`simulation/` (I33).** The adopted row was quoted here for three revisions as
...
**Measured** (`simulation/FINDINGS-adaptive.md` §2, E14/E14b — both `prior`
...
That is reachable two ways and only one of them is an attack. `simulation/FINDINGS-v3.md`
...
> **Implemented at `c2d4407`** (M2.8), to §5.6.1 exactly. `StakeRegistry` carries
> `depositMaintenance` / `propose|cancel|executeMaintenanceWithdrawal`,
> `Moderation` carries `sweepMaintenance`, and the registry's mutation baseline was
> re-established as a **combined** 33/33 rather than left as two partial runs.
> §10's row for this is closed.
...
of a parameter value**, `d` is open (§10), and `simulation/FINDINGS-v3.md` §E
...
SUPER_SAFE  =  verdict == Approve
           ...
           -- the 3/3 conjunct is DROPPED; see below
...
**Decision: it is dropped.** This stood as open on the grounds that removing a
published assurance condition changes what the index promises readers. That reason
does not survive inspection of what the promise currently *is*: `SUPER_SAFE` today
means *"met the real criteria, and then won a coin flip"*, so the label already
promises a random subsample. Dropping the conjunct does not weaken a promise, it
**stops the label lying about its own selectivity** — and it is the only change here
that makes §8.3 agree with its own headline.

The direction is worth being explicit about, because "dropping a safety conjunct"
reads alarming and this one is not: every remaining conjunct is a **tally** fact.
`pooledReject == 0`, `revealCount ≥ SUPER_QUORUM` and `reveals == commits` are what
carry the assurance, they are unchanged, and the content newly admitted is content
that already satisfied all of them. Nothing enters `SUPER_SAFE` that a unanimous,
...
- **It reaches an unrepresentative cohort and not an unlucky draw**, and an
  ...
  Measured (`simulation/FINDINGS-v3.md` §H): at `prior = 0.665`, **82% of false
...
| ~~§8.3's 3/3 conjunct~~ | **DROPPED.** Held open on the grounds that removing a published assurance condition changes what the index promises. That reason does not survive inspection of the promise: `SUPER_SAFE` meant *"met the real criteria, then won a coin flip"*, so the label already published a random subsample and dropping the conjunct **stops it lying about its own selectivity**. Every remaining conjunct is a tally fact and none changed, so nothing enters `SUPER_SAFE` that a unanimous, fully-revealed, super-quorum cohort did not approve |
...
| ~~D3-19: does a successful removal refund, and is there a cooldown~~ | **DECIDED: no refund, and no additional cooldown.** *No refund*, and the reason inverts the intuition: refunding a **successful** removal looks like paying whoever corrects a protocol error, but a removal case is judged by the same engine at the same accuracy, so a removal against **legitimately listed** content carries at the false-approval rate — 60% at `prior` 0.665, `q = 0.30` (`FINDINGS-v3` §A). Refund-on-success therefore makes censorship free in the majority of attempts at the accuracy this design has to assume. **The fee is the only thing pricing a censorship attempt and it must be paid whichever way the case goes.** *No cooldown*, because the engine already bounds repetition without a new knob: a failed removal permanently reserves the `REMOVE` key under §8.4's `REJECTED` row — it is an ordinary claim and was never forked — so a second identical removal is refused outright, and the only recourse is `reopen`, which carries the tally forward and is self-defeating by §8.5's own argument |
...
| ~~`CLAIM_BOUNTY` on `UNRESOLVED`~~ | **DECIDED: it is PAID to whoever poked the terminal transition, on all three rows.** Parked at M2.7 as a fee-schedule change rather than a contradiction — that was the wrong classification. §4.8's own rule is *"a bounty is refunded where the transition it pays for cannot occur, and paid where that transition was performed"*, and on every `UNRESOLVED` row the transition **was** performed: each is permissionless and somebody paid gas to poke it. Retaining the bounty there is not a different fee schedule, it is the same rule applied inconsistently — `DRAW_BOUNTY` already pays exactly that poker on `NO_RANDOMNESS`. **And retaining it is worse than untidy**: `NO_TURNOUT` and `NO_REVEALS` have no party who gains from poking them, so an unpaid poke is a transition nobody is funded to make, on the two rows a thin registry reaches most often |
